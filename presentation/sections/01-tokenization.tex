...
\begin{frame}{Text can be split in different ways}
  ...
  \vspace{-0.3em}
  \begin{exampleblock}{Follow along in the notebook}
    \scriptsize
    Under \textbf{Tokens}, find \emph{An extremely simple tokenizer / de-tokenizer}.
    Its \texttt{stoi} mapping and \texttt{encode} function implement the
    character split above as a first, naive attempt at encoding text as integers.
  \end{exampleblock}
  \vspace{0.5em}
  ...
  {\tiny The subword split above is illustrative; the result depends on the
  tokenizer and its training data.}
\end{frame}
