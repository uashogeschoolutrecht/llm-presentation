...
\begin{frame}{Dot product: scoring a candidate token}
  \small
  Candidate token \(j\) receives the logit
  \(z_j=\mathbf{h}^{\mathsf T}\mathbf{w}_j\): context vector \(\mathbf{h}\) is
  compared with candidate vector \(\mathbf{w}_j\).
  ...
  \begin{alertblock}{Connection to cross-entropy}
    \footnotesize
    Dot products assign scores to candidate tokens. Cross-entropy pushes the
    correct token's score up relative to the competing tokens.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Unembedding: back from vectors to tokens}
  \small
  For now, treat the Transformer as a black box. After ``whatever the
  Transformer does'', it leaves us with an internal vector that represents the
  current context.
  ...
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[stage, fill=tokenblue, draw=tokenblueborder,
        ...
        {one raw score per token\\\textbf{logits}};
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      \begin{block}{Embedding \(W_E\)}
        Token ID \(\longrightarrow{}\) vector
      \end{block}
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{block}{Unembedding \(W_U\)}
        Vector \(\longrightarrow{}\) token scores
      \end{block}
    \end{column}
  \end{columns}

  \begin{alertblock}{Scores are not probabilities}
    Logits are arbitrary real numbers. To predict the next token, we still need
    to turn them into probabilities. That is the job of \textbf{softmax}.
  \end{alertblock}
\end{frame}
...
\begin{frame}{From token vectors to contextual vectors}
  ...
  \begin{alertblock}{The question for the next chapter}
    How can every token exchange information with the other tokens so that its
    vector reflects the surrounding context? Transformers answer this with
    self-attention.
  \end{alertblock}
\end{frame}
