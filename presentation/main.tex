...
\begin{frame}{Follow along in the \texttt{my-llm} notebook}
  The companion notebook builds a small Shakespeare language model. Keep it
  open during the presentation: its three main sections use the same names as
  the chapters in these slides.
      ...
      \begin{block}{Get started}
        \begin{columns}[c,onlytextwidth]
          \begin{column}{0.62\linewidth}
            ...
            \smallskip
            Open \texttt{my-llm.ipynb} and select the project's
            \texttt{.venv} kernel.
          \end{column}
        ...
        \centering
        \scriptsize
        \url{https://github.com/uashogeschoolutrecht/llm-presentation}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Find the code as we go}
        \begin{enumerate}
          \item \hyperlink{sec:tokens}{\textbf{Tokens}}
          \item \hyperlink{sec:embeddings}{\textbf{Vectors and embeddings}}
          \item \hyperlink{sec:attention}{\textbf{Transformers and attention}}
        \end{enumerate}

        \footnotesize
        Run the cells from top to bottom. Start with the character tokenizer,
        then switch to BPE and compare the generated results.

        \smallskip
        \textbf{Tip: show line numbers.} If they are hidden, press
        \texttt{Esc} to enter command mode, then \texttt{Shift+L} to toggle
        them for the whole notebook (Windows and macOS).
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
